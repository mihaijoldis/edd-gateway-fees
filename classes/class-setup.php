...
class EDD_GF_Setup {
	...
	public function is_wp_36_and_edd_activated() {
		...
		} else if ( !class_exists( 'Easy_Digital_Downloads' ) || ( version_compare( EDD_VERSION, '2.4' ) < 0 ) ) {
				if ( is_plugin_active( edd_gf()->basename ) ) {
					deactivate_plugins( edd_gf()->basename );
					unset( $_GET[ 'activate' ] );
					add_action( 'admin_notices', array(
...
?>
	<div class="updated">
		<p><?php
		printf( __( '<strong>Notice:</strong> Easy Digital Downloads Gateway Fees requires Easy Digital Downloads 2.4 or higher in order to function properly.', 'edd_gf' ) );
?>
		</p>
	</div>
	<?php
...
?>
	...
	public function enqueue_scripts() {
		if ( edd_is_checkout() ) {
			...
			wp_enqueue_script( 'gateway-fees', edd_gf_plugin_url . 'assets/js/gateway-fees.js', array( 'jquery' ) );
		}
	}
}
new EDD_GF_Setup;
